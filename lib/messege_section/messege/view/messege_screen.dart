...
class MessegeScreen extends StatelessWidget {
  ...
  Future<String> _getCurrentUserId() async {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('access_token') ?? prefs.getString('token') ?? prefs.getString('access') ?? '';
...
try {
  if (msg.createdAt.isNotEmpty) {
    DateTime parsedDate = DateTime.parse(msg.createdAt).toLocal();
  ...
  log("Time parsing error in MessageScreen: $e");
}
